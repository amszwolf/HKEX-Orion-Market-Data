...
impl PackHead {
    pub fn write_bytes(head: Vec<u8>) -> PackHead {
        PackHead::unpack(write_bytes::<PackHead>(head))
    }
    // pub fn unpack()
...
impl LevelPriceFiller {
    pub fn unpack(rd_buffer: ByteBuffer, filler: u8) -> Vec<Self> {
        ...
        for row in 0..filler {
            level_price_filler.push(LevelPriceFiller {
                ...
                Filler: String::from_utf8(rd_buffer.read_bytes(4 as usize)).unwrap(),
            });
        }
...
impl fmt::Display for LevelPrice {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let mut filler_string = String::new();
        // for row in filler{
        ...
        // for row in self.FillerData{
        //     // filler_string += row.Price.to_string();

        //     println!("{:?},{:?}",row.Price, row.PriceLevel);
        // }


        write!(
            f,
            "MsgSize {} MsgType {}, Filler {}, SecurityCode {}, filler_string {}",
            self.MsgSize,
            self.MsgType,
            self.Filler,
            self.SecurityCode,
...
pub struct IndexData {
    pub MsgSize: u16,
    pub MsgType: u16,
    pub IndexCode: String,
    pub IndexStatus: String,
    pub IndexTime: i64,
    pub IndexValue: i64,
    ...
    ClosingValue: i64,
    PreviousSesClose: i64,
    IndexVolume: i64,
    NetChgPrevDayPct: i32,
    Exception: String,
    Filler: String,
}

impl IndexData {
    pub fn write_bytes(head: Vec<u8>) -> IndexData {
        IndexData::unpack(write_bytes::<IndexData>(head))
    }
    pub fn unpack(rd_buffer: ByteBuffer) -> IndexData {
        let mut rd_buffer = rd_buffer;
        IndexData {
            MsgSize: rd_buffer.read_u16(),
            MsgType: rd_buffer.read_u16(),
            IndexCode: String::from_utf8(rd_buffer.read_bytes(11 as usize)).unwrap(),
            IndexStatus: String::from_utf8(rd_buffer.read_bytes(1 as usize)).unwrap(),
            IndexTime: rd_buffer.read_i64(),
            IndexValue: rd_buffer.read_i64(),

            NetChgPrevDay: rd_buffer.read_i64(),
            HighValue: rd_buffer.read_i64(),
            LowValue: rd_buffer.read_i64(),
            EASValue: rd_buffer.read_i64(),
            IndexTurnover: rd_buffer.read_i64(),
            OpeningValue: rd_buffer.read_i64(),

            ClosingValue: rd_buffer.read_i64(),
            PreviousSesClose: rd_buffer.read_i64(),
            IndexVolume: rd_buffer.read_i64(),
            NetChgPrevDayPct: rd_buffer.read_i32(),
            Exception: String::from_utf8(rd_buffer.read_bytes(1 as usize)).unwrap(),
            Filler: String::from_utf8(rd_buffer.read_bytes(3 as usize)).unwrap(),
        }
    }
}
...
pub struct BrokerQueue {
    pub MsgSize: u16,
    pub MsgType: u16,       //int16 2 Type of message. 54 Broker Queue
    pub SecurityCode: u32, // Uint32 4 Uniquely identifies a security available for tradin 5 digit security codes with possible values 1 – 99999
    pub ItemCount: u8, //Uint8 1 This field contains the number of items in the message – repeating items are shown indented below. 0 to 40
    pub Side: u16,     //Uint16 2 Side of the order 1 Buy 2 Sell
    pub BQMoreFlag: String, // 1 Flag indicating if there are more
    // broker numbers in the queue
    // Y More broker numbers exist in the
    // queue
    // N No more exist
    pub Item: u16, // Uint16 2
    // This field contains either the
    // broker number or the number of
    // spreads away from the best
    // price.
    pub Type: String, //1 Indicates the type of information
...
impl BrokerQueue {
    pub fn write_bytes(head: Vec<u8>) -> Self {
        Self::unpack(write_bytes::<Self>(head))
    }

    pub fn unpack(rd_buffer: ByteBuffer) -> BrokerQueue {
        let mut rd_buffer = rd_buffer;
        BrokerQueue {
            MsgSize: rd_buffer.read_u16(),
            MsgType: rd_buffer.read_u16(),
            SecurityCode: rd_buffer.read_u32(),
            ItemCount: rd_buffer.read_u8(),
            Side: rd_buffer.read_u16(),
            BQMoreFlag: String::from_utf8(rd_buffer.read_bytes(1 as usize)).unwrap(),
            Item: rd_buffer.read_u16(), // Uint16 2
            Type: String::from_utf8(rd_buffer.read_bytes(1 as usize)).unwrap(), //1 Indicates the type of information
            // Filler: String::from_utf8(rd_buffer.read_bytes(1 as usize)).unwrap(), //1
        }
    }
}
...
pub struct LiquidityProvider {
    pub MsgSize: u16,              // Uint16  2 Size of the message
    pub MsgType: u16,              // Uint16 2 Type of message. 13 Liquidity Provider
    pub SecurityCode: u32, // Uint32 4 Uniquely identifies a security available for trading 5 digit security codes with possible values 1 – 99999
    pub NoLiquidityProviders: u16, // Uint16 2 Number of liquidity providers within this message. 1 to 50 10
    pub LPBrokerNumber: u16,       // Uint16 2 Broker number of the liquidity provider
}

impl LiquidityProvider {
    // pub fn write_bytes()
    pub fn write_bytes(head: Vec<u8>) -> LiquidityProvider {
        LiquidityProvider::unpack(write_bytes::<LiquidityProvider>(head))
    }

    pub fn unpack(rd_buffer: ByteBuffer) -> LiquidityProvider {
        let mut rd_buffer = rd_buffer;
        LiquidityProvider {
            MsgSize: rd_buffer.read_u16(),
            MsgType: rd_buffer.read_u16(),
            SecurityCode: rd_buffer.read_u32(),
            NoLiquidityProviders: rd_buffer.read_u16(),
            LPBrokerNumber: rd_buffer.read_u16(),
        }
    }
}
